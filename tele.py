...
class TelegramAccountManager:
    ...
    def _init_db(self):
        with sqlite3.connect(self.accounts_db) as conn:
            cursor = conn.cursor()
            cursor.execute('''CREATE TABLE IF NOT EXISTS accounts
                            (phone TEXT PRIMARY KEY, api_id INTEGER, api_hash TEXT, session_path TEXT)''')
            conn.commit()
    ...
    def _init_dm_db(self):
        with sqlite3.connect(self.dm_db, timeout=10) as conn:
            ...
            cursor.execute('''CREATE TABLE IF NOT EXISTS dmed_users
                            (user_id TEXT PRIMARY KEY, timestamp DATETIME)''')
            conn.commit()
    ...
    try:
        ...
        while True:
            ...
            if choice == 1:
                ...
                if members:
                    filename = f"members_{entity}_{datetime.now().strftime('%Y%m%d')}.txt"
                    os.makedirs('scraped_users', exist_ok=True)
                ...
                if user_file:
                    ...
                    if selected_accounts:
                        if len(selected_accounts) == 1:
                            ...
                            sent = await send_direct_messages_multi_account(account_manager, selected_accounts, user_file, message)
